...
from utils_zuco import load_embeddings_file, get_data
from constants import POS, CHUNK, NER, SRL, SENTI2, SENTI3, NFIX, REX, EEG_T, EEG_A, EEG_B, EEG_G, FIXP, FFD, FREQ, TRT, MFD, ALL, MODEL_FILE, PARAMS_FILE, \
...
class SluiceNetwork(object):
        ...
        for epoch in range(num_epochs):
            ...
            for (word_indices, char_indices), task2label_id_seq in train_data:
                ...
                for task, y in task2label_id_seq.items():
                    if task in [POS, NER, SENTI2, SENTI3, REX, NFIX, FFD, TRT, MFD, EEG_T, EEG_A, EEG_B, EEG_G, FIXP, FREQ, ALL]:
    ...
    def predict(self, features, task_name, train=False):
        ...
        penalty = dynet.parameter(self.subspace_penalty)
        #print("PENALTY")
                ...
                if self.num_subspaces == 2 and self.constraint_weight != 0:
                    ...
                    for param_idx in range(len(lstm_parameters)):
                        if param_idx in self.constrain_matrices:
                            ...
                            W_subspaces = dynet.reshape(W, (self.num_subspaces, W_shape[0] / float(self.num_subspaces), W_shape[1]))
    ...
    def evaluate(self, test_X, test_Y, evaluating_fold=False, predictions_file=None, main_task=None):
        """
        ...
        if self.cross_stitch:
            for layer_num in range(self.h_layers):
                ...
                print('Cross-stitch unit values (alpha) at layer %d.' % layer_num,
                      end=' ', flush=True)
        ...
        if self.layer_connect == STITCH:
            for task_id, task_name in enumerate(self.task_names):
                ...
                print('Layer-stitch unit values (beta) for task %s: %s.'
        ...
        for i, ((word_indices, word_char_indices), task2label_id_seq)\
            ...
            for task, label_id_seq in task2label_id_seq.items():
                ...
                if evaluating_fold is True:
                        #print("-------------PRINTING TO Predictions FILE----------")
                        if task == main_task:
                            with open(predictions_file, "a") as predfile:
                                for gold, predicted in zip(label_id_seq, predicted_label_indices):
                                    predfile.write(str(gold) + "\t" + str(predicted) + "\n")
