...
def main(args):
    ...
    if not os.path.exists(args.result_file):
        print('Result file exists %s...' % args.result_file, flush=True)
        #open(args.result_file, 'a').close()
    if not os.path.exists(args.pred_file):
        print('Result file exists %s...' % args.pred_file, flush=True)
    ...
    for i, test_domain in enumerate(args.test):
        ...
        test_X, test_Y, _, _, _, _, _ = utils_zuco.get_data(
        ...
        test_accuracy = model.evaluate(test_X, test_Y, evaluating_fold=True, predictions_file=args.pred_file, main_task=model.main_task)
        ...
        with open(args.result_file, "a") as resfile:
            print("-------------PRINTING TO RESULT FILE----------")
            resfile.write('%.4f \n' % test_accuracy)

        log_file = os.path.join(args.log_dir, 'log.txt')
        utils_zuco.log_score(log_file, args.train, test_domain, test_accuracy,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dynet-devices', type=str, help='CPU/GPU devices to use')
    ...
    parser.add_argument('--result-file', required=True,
                        help='the file where the finale results are logged')
    parser.add_argument('--pred-file', required=True,
                        help='the file where the finale predictions are printed')
